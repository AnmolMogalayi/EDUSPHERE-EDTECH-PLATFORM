...
const NavBar = ({ setProgress }) => {
    ...
    return (
        <div className={`flex sm:relative w-screen relative z-50 h-14 items-center justify-center border-b-[1px] border-b-richblack-700 transition-all duration-500`} 
            style={{ background: 'linear-gradient(270deg, #232526, #414345, #232526, #232526, #414345)', backgroundSize: '1000% 1000%', animation: 'navbarGradientMove 8s ease infinite' }}>
                                ...
                                <li key={index} >
                                    {
                                        element.title === "Catalog" ? (<div className=' flex items-center group relative cursor-pointer'>
                                            ...
                                            <div className='invisible absolute left-[50%] top-[50%] z-[1000] flex w-[200px] translate-x-[-50%] translate-y-[3em] flex-col rounded-lg p-4 text-richblack-900 opacity-0 transition-all duration-150 group-hover:visible group-hover:translate-y-[1.65em] group-hover:opacity-100 lg:w-[300px]'
                                                style={{ background: 'linear-gradient(135deg, #a1c4fd 0%, #c2e9fb 100%)' }}>
                                                <div className='absolute left-[50%] top-0 -z-10 h-6 w-6 translate-x-[80%] translate-y-[-40%] rotate-45 select-none rounded' style={{ background: 'linear-gradient(135deg, #a1c4fd 0%, #c2e9fb 100%)' }}></div>
                                                {
                                                    sublinks?.length < 0 ? (<div></div>) : (
                                                        sublinks?.map((element, index) => (
